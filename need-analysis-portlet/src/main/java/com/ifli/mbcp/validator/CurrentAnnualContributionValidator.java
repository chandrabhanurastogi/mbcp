...
@Component("currentAnnualContributionValidator")
public class CurrentAnnualContributionValidator implements Validator {
	//private static final Logger logger = LoggerFactory.getLogger(NeedAnalysisFormValidator.class);
	private static final Logger _log = LoggerFactory.getLogger(CurrentAnnualContributionValidator.class);
	...
	public void validate(Object target, Errors errors) {
		...
		List<NeedsAnalysisVO> na = needs.getNeedsAnalysis();
		System.out.println("sizeeeee---"+na.size());
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "needsAnalysis[0].annualAffordableContribution", "NotBlank.annualAffordableContribution");
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "needsAnalysis[0].yearsOfContribtion", "NotBlank.yearsOfContribution");
		ValidationUtils.rejectIfEmptyOrWhitespace(errors,"needsAnalysis[0].annualContributionIncrement", "NotBlank.annualContributionIncrement");
		//ValidationUtils.rejectIfEmptyOrWhitespace(errors,"needsAnalysis[${loop.index}].annualContributionIncrement", "NotBlankAnnualContributionIncrement");
		...
		_log.info("inside currentAnnualContributionValidator Validator");
	}
